function [Bcart] = Validation_Bandit_Cac_UVs(Xin,Yin,Zin,L,R,M)
...
for n_axi = 1:2
    ...
    for ycount = 1:size(Yin,2)
        ...
        for zcount = 1:size(Zin,2)
            ...
            Bcart(ycount,zcount,2,n_axi) = (Bcartpri(ycount,zcount,1,n_axi).*sin(phiM) + Bcartpri(ycount,zcount,2,n_axi).*cos(phiM));
            Bcart(ycount,zcount,3,n_axi) = (Bcartpri(ycount,zcount,3,n_axi));
