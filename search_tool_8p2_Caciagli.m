<<<<<<< HEAD
function [SaveVar,varst] = search_tool_8p2_Caciagli(KRV,RES,pm_cl,Lengths,theta,Yin,Zin,s_rad,con, sigmoid_pc)

%% corrected version of 7p4

% this version of the code allows for multiple inputs on all variables
% (including s_rad) for the non-idealised code. Sigmoids are integrated
% here, allowing for another parameter to evaluate)
...
SWres = zeros(size(KRV,2),ntestmax,length(pm_cl),length(RES),length(Lengths),length(s_rad),length(sigmoid_pc)); 
                ...
                for  count2 = 1:size(KRV,2)
                    ...
                    for scount = 1:size(s_rad,2)
                        ...
                        while abs(tmps(1) - tmps(2)) > 5e-3 && SH0 > MxB(pmcount,size(MxB,2),Lcount) && tmps(2) ~= 0  
                            ...
                            SWres(count2,count+1,pmcount,rescount,Lcount,scount,pccount) = SWnext;
                            FWHMres(count2,count,pmcount,rescount,Lcount,scount,pccount) = FWHMX(1); % if +1 not needed then can use FWHMres(:,1,:,:) = [];
                            ind1res(count2,count,pmcount,rescount,Lcount,scount,pccount)= indout(1);
                            ind2res(count2,count,pmcount,rescount,Lcount,scount,pccount) = indout(2);
                            Bset(count2,count,pmcount,rescount,Lcount,scount,pccount) = MxB(pmcount,pzcut,Lcount);
                            SHo(count2,count,pmcount,rescount,Lcount,scount,pccount) = SH0;
                            MLOCa(count2,count,pmcount,rescount,Lcount,scount,pccount) = MLOC(1);
