function [SaveVar,varst] = search_tool_9p1_Caciagli(KRV,RES,pm_cl,Lengths,theta,Yin,Zin,s_rad, IB ,con, min_separation)

%% Corrected version of 7p3 

% this version of the code allows for multiple inputs on all variables
% (including s_rad) for the non-idealised code. 

% corrected for the artificial suppression of maximum number of channels by
% the while loop in lines 160-183

% corrected for the artificial increase in KRV using the median field line
% in lines 147-149

% corrected FWHMNVC function too, but this is only a part of this function.
% But now uses the real mid point (NSA  = 0.5) instead of the middle
% between start and end points. used in line 145
...
SWres = zeros(size(KRV,2),ntestmax,length(pm_cl),length(RES),length(Lengths),length(s_rad), length(IB)); 
            ...
            for pmcount = 1:length(pm_cl)
                ...
                for  count2 = 1:size(KRV,2)
                    ...
                    for scount = 1:size(s_rad,2)
                        ...
                        for count3 = 1:size(IB,2)
                            
                            for count4 = 1:size(min_separation,2)
                                ...
                                while abs(tmps(1) - tmps(2)) > min_separation(count4) && SH0 > MxB(pmcount,size(MxB,2),Lcount) && tmps(2) ~= 0  
                                    % 
                                    ...
                                    FWHMres(count2,count,pmcount,rescount,Lcount,scount,count3,count4) = FWHMX(1); % if +1 not needed then can use FWHMres(:,1,:,:) = [];
                                    ind1res(count2,count,pmcount,rescount,Lcount,scount,count3,count4)= indout(1);
                                    ind2res(count2,count,pmcount,rescount,Lcount,scount,count3,count4) = indout(2);
                                    Bset(count2,count,pmcount,rescount,Lcount,scount,count3,count4) = MxB(pmcount,pzcut,Lcount);
                                    SHo(count2,count,pmcount,rescount,Lcount,scount,count3,count4) = SH0;
