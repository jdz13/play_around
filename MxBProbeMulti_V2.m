function [MxB, probe_line] = MxBProbeMulti_V2(M,OD, L,D_prac,N_probe, IB)
...
MxB = zeros(length(OD),size(probe_line,2),length(L));
    ...
    for m = 1:length(OD)
        for o = 1:length(L)
           ...
           [MxB(m,n,o)] = (Hrho(M, probe_line(m,n), 0, 0, L(o)/2, OD(m)/2)- Hrho(M, probe_line(m,n), 0, 0, L(o)/2, IB/2))*mu0;
