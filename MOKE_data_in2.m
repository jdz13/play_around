function[Data_str, fit_results] = MOKE_data_in2(figno, ind, Tpt)
    ...
    B_mat = zeros(ind(2)-ind(1)+1,length(zz)-2); Moke_data = B_mat; 
...
%         ----------------------------------------------------------------
        ...
        [newnormy(:,p), waGrad, Sfactor] = MOKE_norm_WA_tool( Normset(:,p),B_mat(:,p),fitresult);
        
        % Fit model to data.
        [xData, yData] = prepareCurveData( B_mat(:,p), newnormy(:,p) );
...
%     title 'Comparing c values';
    xlabel 'Platinum thickness t_P_t [nm]'; ylabel 'c [Oe]'
    hold on
...
%     title 'Comparing a values';
    ...
    hlegend = legend (legendCell, 'Location', 'Southeast');
