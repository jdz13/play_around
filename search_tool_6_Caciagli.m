function [SaveVar] = search_tool_6_Caciagli(KRV,RES,pm_cl,Lengths,theta,Yin,Zin,s_rad,con, sigmoid_pc)
...
D_prac = 10e-2; N_probe = 10001; M = 1e6; IB = 6e-3; sigmoid.pc = sigmoid_pc;
            ...
            for  count2 = 1:size(KRV,2)
                ...
                while abs(tmps(1) - tmps(2)) > 1e-4 && SH0 > MxB(pmcount,size(MxB,2),Lcount) && tmps(2) ~= 0  
                    ...
                    for pull = 1:length(theta) 
                       ...
                       sigmoid.c = swinit; 
                       sigmoid.a = 200*pi/sigmoid.pc/sigmoid.c;
