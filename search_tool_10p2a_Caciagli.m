function [SaveVar,varst] = search_tool_10p2a_Caciagli(KRV,RES,pm_cl,Lengths,theta,Yin,Zin,s_rad,con,a)
...
SWres = zeros(size(KRV,2),ntestmax,length(pm_cl),length(RES),length(Lengths),length(s_rad),length(a)); 
                        ...
                        for acount = 1:size(a,2)
                            ...
                            while abs(tmps(1) - tmps(2)) > min_separation && swinit > min_separation && tmps(2) ~= 0  
                                ...
                                for pull = 1:length(theta) 
                                   ...
                                   BZM = sigmf(Bxnew,[a_use.*1e4,swinit]);
                                ...
                                ind1res(count2,count,pmcount,rescount,Lcount,scount,acount)= indout(1);
                                ind2res(count2,count,pmcount,rescount,Lcount,scount,acount) = indout(2);
                                Bset(count2,count,pmcount,rescount,Lcount,scount,acount) = MxB(pmcount,pzcut,Lcount);
                                SHo(count2,count,pmcount,rescount,Lcount,scount,acount) = SH0;
